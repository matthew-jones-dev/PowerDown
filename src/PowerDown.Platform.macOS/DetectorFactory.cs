...
public class MacDetectorFactory : IDetectorFactory
{
    private readonly ISteamPathDetector _steamPathDetector;

    public MacDetectorFactory(ISteamPathDetector steamPathDetector)
    {
    ...
    public List<IDownloadDetector> CreateDetectors(Configuration config, ILogger logger)
    {
        ...
        if (config.MonitorSteam)
        {
            ...
            if (steamPath != null)
            {
                var detector = new MacSteamDownloadDetector(steamPath, logger);
